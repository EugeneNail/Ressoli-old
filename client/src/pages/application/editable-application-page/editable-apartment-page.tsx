...
type EditableApartmentPageProps = {
  willCreate?: boolean;
};

function EditableApartmentPage({ willCreate }: EditableApartmentPageProps) {
  ...
  const [options, setOptions] = useState(new ApplicationOptions<ApartmentOptions>());
  const [isLoading, setLoading] = useState(false);

  useEffect(() => {
    if (!willCreate) {
      setLoading(true);
      api.get<{ data: Application<Apartment> }>(`/applications/${id}/edit`).then(({ data: { data } }) => {
    ...
    api.get("/options/application?type=apartment").then((response) => {
  ...
  async function persistApartment() {
    const response = await api.post("/apartments", apartment.fields);

    if (response.status === 422) {
      apartment.setErrors(response.data.errors);
      return;
    }

    if (response.status === 201 || response.status == 200) {
      apartment.fields.id = response.data;
    }
  ...
  async function checkContractValidity() {
    ...
    submitApartmentApplication();
  }

  async function submitApartmentApplication() {
  ...
  const { steps, back, next, currentStep, goTo } = useMultiStepForm([
    ...
    <ApartmentForm options={options.applicable} back={() => back()} submit={persistApartment} state={apartment} />,
...
export default EditableApartmentPage;
